...
function queuePrinter(bufferSize, capacities, documents) {
  // 작업 목록 = 배열, 각칸은 한 개의 문서만 위치
  // 1초에 한 칸씩 이동 가능
  // bufferSize = 작업목록의 크기,  // capacities = 최대 용량

  // 작업목록 = []
  // 최종값 = 1초씩 count

  let inWork = [];
  let seconds = 0; // bufferSize = 2 / capacities = 10 / documents = [7, 4, 5 ,6]
  let sumInWork;
  // bufferSize = [0, 0]
  // 1초가 지나면 inWork [0, 7]
  // 2초가 지나면 inWork [7, 0];
  ...
  for (let i = 0; i < bufferSize; i++) {
    inWork.push(0); // inWork = [0]
  }

  inWork.push(documents.shift()); // 1초 > inWork = [0, 7]
  while (sumInWork !== 0) {
    seconds++;
    inWork.shift();
    sumInWork = inWork.slice(1).reduce(function (acc, cur) {
      return acc + cur;
    }, 0);
    if (sumInWork + documents[0] <= capacities) {
      // inWork = [7, 0]
      inWork.push(documents.shift());
    } else {
      inWork.push(0);
    }
    sumInWork = inWork.reduce(function (acc, cur) {
      return acc + cur;
    });
  }
  return seconds;
